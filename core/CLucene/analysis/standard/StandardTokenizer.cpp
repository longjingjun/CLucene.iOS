...
Token* StandardTokenizer::next(Token* t) {
    ...
    while (!EOS) {
        ...
        if(!EOS && DIGIT){
            StringBuffer myStr = getFollowingStringWithoutReadChar();
            //printf("myStr: %ls\r\n", myStr.toString());
            for(int i=0;i<myStr.length();i++){
                char tempCh = myStr.charAt(i);
                if(_istdigit(tempCh) == 0){
                    //printf("isAllDigit = false\r\n");
                    isAllDigit = false;
                    break;
                }
            }
        ...
        if (ALPHA || UNDERSCORE || !isAllDigit) {
            //printf("ALPHA || UNDERSCORE\r\n");
            tokenStart = rdPos;
            t = ReadAlphaNum(ch,t);
            if ( t != NULL) return t;
        } else if (DIGIT || NEGATIVE_SIGN_ || DECIMAL) {
            //printf("DIGIT || NEGATIVE_SIGN_ || DECIMAL\r\n");
	  ...
	  if (  str.len < LUCENE_MAX_WORD_LEN ){
          ...
          if (!EOS && str.len < LUCENE_MAX_WORD_LEN-1 ) { //still have space for 1 more character?
			  switch(ch) { /* What follows the first alphanum segment? */
                  ...
                  case '_':
                      return processPunctuation(&str,t,ch);
                  case '-':
                      return processPunctuation(&str,t,ch);
                  case '/':
                      return processPunctuation(&str,t,ch);
  ...
  Token* StandardTokenizer::processPunctuation(StringBuffer *str, Token* t, int ch){
      StringBuffer myStr = getFollowingStringWithoutReadChar();
      //printf("myStr2: %ls\r\n", myStr.toString());
      
      if(hasDigit(str) || hasDigit(&myStr)){
          str->append(myStr.getBuffer());
          for(int i=0;i<myStr.length();i++){
              readChar();
          }
          return setToken(t,str,CL_NS2(analysis,standard)::NUM);
      }else{
          return setToken(t,str,CL_NS2(analysis,standard)::NUM);
      }
  }

  StringBuffer StandardTokenizer::getFollowingStringWithoutReadChar(){
      CL_NS(util)::StringReader *stringReader = (CL_NS(util)::StringReader*)(rd->input);
      const TCHAR *tValue = stringReader->getValue();
      
      int64_t tPos = rd->input->getPosition();
      tPos--;//position 1 means read tValue[0];
      
      int64_t tSize = rd->input->size();
      StringBuffer myStr(tSize);
  ...
  bool StandardTokenizer::hasDigit(StringBuffer *str){
      size_t length = str->length();
      for(int i=0;i<length;i++){
          TCHAR ch = str->charAt(i);
          if(DIGIT){
              return true;
          }
      }
